...
export class OStorageService {
    ...
    constructor(cfg: any, private logger: any) {
        this.ossClient = new aws.S3(cfg.client);
        this.Url = cfg.url;
            ...
            if (value != null) {
                ...
                if (AllObjects != null) {
                    for (let eachObj of AllObjects) {
                        ...
                        const url = 'http://localhost:5000/' + value + '/' + meta.key;
                        const filename = meta.filename;
                        const owner = JSON.parse(meta.meta);
                        let object = { file_name: filename, url: url, meta: owner };
    ...
    async get(call: Call<GetRequest>, context?: any): Promise<any> {
        ...
        const url = 'http://localhost:5000/' + bucket + '/' + meta_key.Metadata.key;
        return {
            ...
            uploadable
                .on('httpUploadProgress', (chunk) => {
                    if (chunk.loaded == chunk.total) {
                        this.logger.info(`Successfully persisted object ${key} in bucket ${bucket}`);
                        resolve();
                    }
                })
                .send();
        });
        return 'http://localhost:5000/' + bucket + '/' + uid;
    }
